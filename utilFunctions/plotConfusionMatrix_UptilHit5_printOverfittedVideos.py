...
###
# File: plotConfusionMatrix_UptilHit5_printOverfittedVideos.py
# Author: Dhruv Jain 
# Plots predictions for every class and compares them against the true labels for that class (this is not to be confused with general CM which is not valid for multi-label classification).
# 
...
for i in range(1, predicts.shape[0]):
    ...
    # CODE FOR HIT 2
    if(true_label[total] != predict_label[total] and int(sys.argv[4]) >= 2):
        predict_label[total] = int(predicts[i][1].split(' ')[2])            # assign the second top label to predict_label[total] 
        true_label[total] = searchLabel(true[index], predict_label[total])  # search if that predicted label exists in the original labels of the video and assign accordingly. 
    
    # CODE FOR HIT 3
    if(true_label[total] != predict_label[total] and int(sys.argv[4]) >= 3):
        predict_label[total] = int(predicts[i][1].split(' ')[4])            # assign the third top label to predict_label[total] 
        true_label[total] = searchLabel(true[index], predict_label[total])  # search if that predicted label exists in the original labels of the video and assign accordingly. 
    
    # CODE FOR HIT 4
    if(true_label[total] != predict_label[total] and int(sys.argv[4]) >= 4):
        predict_label[total] = int(predicts[i][1].split(' ')[6])            # assign the third top label to predict_label[total] 
        true_label[total] = searchLabel(true[index], predict_label[total])  # search if that predicted label exists in the original labels of the video and assign accordingly. 

    # CODE FOR HIT 5
    if(true_label[total] != predict_label[total] and int(sys.argv[4]) >= 5):
        predict_label[total] = int(predicts[i][1].split(' ')[8])            # assign the third top label to predict_label[total] 
        true_label[total] = searchLabel(true[index], predict_label[total])  # search if that predicted label exists in the original labels of the video and assign accordingly. 

    if(true_label[total] != predict_label[total]):
        predict_label[total] = int(predicts[i][1].split(' ')[0])           # if Hits still doesn't match, just assign the top label
